...
#TODO: Understand exactly what is going on here with the scaling
function compute_E(x, rho, kx)
    Nx = length(x)
    #TODO: check this
    #####################################################
    # delta_x = x[2] - x[1]                             #
    # scaling_factor = 2*pi / (x[end] + delta_x - x[1]) #
    #                                                   #
    # k[1] = 1.0                                        #
    #####################################################
    kx_temp = copy(kx)
...
f, Energy = vlasov_ampere(Nx, Nv, x_min, x_max, v_min, v_max, t_max, Nt)
...
savefig(EE_plot, "VAmp1DFourier.pdf")
